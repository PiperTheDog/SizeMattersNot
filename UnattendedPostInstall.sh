...
#Modified Script for fully automated proxmox deployments
#The Script does the following: 
# - Most Whiptail blocks have been removed and most blocks have been hardcoded 
# - Please note, DO NOT USE THIS SCRIPT WITHOUT REVIEWING IT YOURSELF FIRST 



echo "DO NOT USE THIS SCRIPT WITHOUT REVEIWING IT FIRST" 
...
header_info() {
  clear
  echo -e "${GN}Proxmox VE 9 Post Install Script${CL}"
  echo -e "${YW}Running post-install routines...${CL}"
}
...
start_routines_9() {
  ...
  if find /etc/apt/sources.list.d/ -maxdepth 1 -name '*.sources' | grep -q .; then
    #Replace whiptail with a terminal message so the script doesn't pause
    msg_ok "Modern deb822 sources (*.sources) already exist."
  else
...
# ---- PVETEST ----
  if component_exists_in_sources "pve-test"; then
    ...
    msg_ok "Skipping 'pve-test' repository (Automated)"
  fi
